...
export async function getDoctors(){
    try {
        ...
        throw new Error("Failed to fecth doctors")
    }
}
    ...
    try {
        if(!input.name || !input.email) throw new Error("Name and email are required")
         ...
         const doctor = await prisma.doctor.create({
            ...
            revalidatePath("/admin") // dar um refresh depois de criar o doutor
        ...
        if (error?.code === "P2002") {
            throw new Error("A doctor with this email exists")
        }
...
export async function updateDoctor(input:UpdateDoctorInput){
    try {
        ...
        if(!input.name || !input.email) throw new Error("Name and email are required")
            ...
            //Procurando o doctor que queremos atualizar ou seja o doutor corrente
